...
class ActionLayer(BaseActionLayer):
    ...
    def _inconsistent_effects(self, actionA, actionB):
        ...
        for actionA_effect in self.children[actionA]:
            for actionB_effect in self.children[actionB]:
                if actionA_effect == ~actionB_effect:
                    return True
    ...
    def _interference(self, actionA, actionB):
        ...
        # TODO: implement this function
        for actionA_effect in self.children[actionA]:
            for actionB_preCon in self.parents[actionB]:
                if actionA_effect == ~actionB_preCon:
                    return True
        
        for actionB_effect in self.children[actionB]:
            for actionA_preCon in self.parents[actionA]:
                if actionB_effect == ~actionA_preCon:
                    return True
    ...
    def _competing_needs(self, actionA, actionB):
        ...
        """
        # TODO: implement this function
        for actionA_preCons in self.parents[actionA]:
            for actionB_preCons in self.parents[actionB]:
                if self.parent_layer.is_mutex(actionA_preCons, actionB_preCons):
                    return True
...
class PlanningGraph:
    ...
    def h_levelsum(self):
        ...
        # TODO: implement this function     
        #   
        return sum(self.getLevelCosts())
        
    def getLevelCosts(self):
        goalsToCheck = self.goal.copy()
        costs = []
        level = 1
        while not self._is_leveled:
            self._extend()
            for goal in goalsToCheck.copy():
                if goal in self.literal_layers[level-1]:
                    costs.append(level-1)
                    goalsToCheck.remove(goal)
            if len(goalsToCheck) == 0:
                break
            level += 1
        return costs
    ...
    def h_setlevel(self):
        ...
        while not self._is_leveled:
            ...
            allGoalsMet = self.checkAllGoalsMet(currentLayer)
            if allGoalsMet:
                goalsMutex = self.checkGoalsMutex(currentLayer)
                if not goalsMutex:
                    return currentLayer
            level += 1

    def checkAllGoalsMet(self, level):
        allGoalsMet = True
        for goal in self.goal:
            if goal not in self.literal_layers[level]:
                allGoalsMet = False
        return allGoalsMet

    def checkGoalsMutex(self, level):
        goalsMutex = False
        for goalA in self.goal:
            for goalB in self.goal:
                if self.literal_layers[level].is_mutex(goalA, goalB):
                    goalsMutex = True
        return goalsMutex
...
# maxLevels = 1
# costs = []
# while len(goalsToCheck) > 0:
#     self.fill(maxLevels)
#     for goal in goalsToCheck.copy():
#         if goal in self.literal_layers[maxLevels-1]:
#             costs.append(maxLevels-1)
#             goalsToCheck.remove(goal)
#     maxLevels += 1
# return costs
        ...
        # self.fill()
        # print("Layers "+str(len(self.literal_layers)))
        # costs = []
        # for goal in self.goal:
    ...
    #     for key,value in enumerate(self.literal_layers):
    #         if goal in value:
    #             return key
